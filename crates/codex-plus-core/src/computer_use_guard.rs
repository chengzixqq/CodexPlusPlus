...
const INSTALLED_BY_DEFAULT_BUNDLED_PLUGINS: &[&str] = &["browser", "chrome", "computer-use"];
...
pub(crate) fn ensure_openai_bundled_marketplace(home: &Path) -> anyhow::Result<Option<PathBuf>> {
    ...
    if is_complete_openai_bundled_marketplace(&active) {
        ensure_openai_bundled_marketplace_install_policies(&active)?;
    ...
    if let Some(configured) = configured_openai_bundled_marketplace(home) {
        if is_complete_openai_bundled_marketplace(&configured) {
            ensure_openai_bundled_marketplace_install_policies(&configured)?;
    ...
    match replace_active_marketplace(&active, &staging) {
        ...
        Err(_) if is_complete_openai_bundled_marketplace(&staging) => {
            ...
            ensure_openai_bundled_marketplace_install_policies(&staging)?;
            Ok(Some(staging))
        }
...
#[cfg(windows)]
fn ensure_openai_bundled_marketplace_install_policies(path: &Path) -> anyhow::Result<()> {
    ...
    for entry in plugins {
        ...
        if !INSTALLED_BY_DEFAULT_BUNDLED_PLUGINS.contains(&name) {
            continue;
        }
        if !entry
            .get("policy")
        ...
        {
            entry["policy"]["installation"] = serde_json::json!("INSTALLED_BY_DEFAULT");
            changed = true;
        }
        if entry["policy"].get("authentication").is_none() {
            entry["policy"]["authentication"] = serde_json::json!("ON_INSTALL");
            changed = true;
        }
    }
    if changed {
        ...
        updated.push('\n');
        atomic_write_runtime_file(&manifest_path, updated.as_bytes())?;
    }
    Ok(())
}
...
fn bundled_marketplace_json() -> String {
    ...
    .into_iter()
    .map(|(name, category)| {
        let installation = if INSTALLED_BY_DEFAULT_BUNDLED_PLUGINS.contains(&name) {
...
"#,
    ...
    fn ensure_openai_bundled_marketplace_rebuilds_damaged_active_from_cache() {
        ...
        let marketplace_json: serde_json::Value = serde_json::from_str(&marketplace).unwrap();
        for plugin in INSTALLED_BY_DEFAULT_BUNDLED_PLUGINS {
            assert_eq!(
                bundled_plugin_installation_policy(&marketplace_json, plugin).as_deref(),
                Some("INSTALLED_BY_DEFAULT")
            );
        }
        for plugin in BUNDLED_MARKETPLACE_PLUGINS {
    ...
    #[cfg(windows)]
    #[test]
    fn ensure_openai_bundled_marketplace_repairs_complete_manifest_install_policies() {
        let temp = tempfile::tempdir().unwrap();
        let home = temp.path();
        let active = home
            .join(".tmp")
        ...
        std::fs::write(
            active
                ...
                .join("plugins")
                .join("marketplace.json"),
            bundled_marketplace_json().replace("INSTALLED_BY_DEFAULT", "AVAILABLE"),
        )
        .unwrap();
        for plugin in BUNDLED_MARKETPLACE_PLUGINS {
        ...
        assert_eq!(
            bundled_plugin_installation_policy(&marketplace_json, "latex").as_deref(),
            Some("AVAILABLE")
        );
    }
    ...
    fn bundled_plugin_installation_policy(
        ...
        marketplace["plugins"]
            .as_array()?
            .iter()
            .find(|entry| entry["name"].as_str() == Some(plugin))?
            .get("policy")?
            .get("installation")?
            .as_str()
